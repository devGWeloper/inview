...
- **모델별 요청 대비 실패** (`ModelFailureTiles`) — 모델 하나가 카드 하나다.
  ...
  - **막대 눈금은 카드마다 닫혀 있다** — 트랙 전체 = 그 모델의 전체 호출, 채움 = 실패분.
    격자 전체가 눈금을 공유하면(최악 = 꽉 참) 28.6% 인 모델이 100% 채워져 "전부 실패" 로 읽힌다.
    기준선이 표본에 따라 움직이지 않아야 카드끼리도 그대로 견줄 수 있다
  - 숫자 색도 같은 이유로 **절대 임계**다 — `RATE_CRIT`(10%) 빨강 · `RATE_WARN`(3%) 앰버 ·
    실패 0건 초록. "가장 나쁜 놈" 강조는 조회 범위가 바뀌면 따라 바뀐다
...
- **노드별·모델별·키별·사용자별 분포** (`DimCard`) — 키별(`KEY_NM`)은 `keyAvailable` 일 때만.
  카드 수가 3↔4 로 바뀌므로 `.to-grid` 는 `auto-fit` 이다
- **자주 발생한 오류 사유** (`ReasonList`) — `ERR_CTN` 앞 100자로 클러스터링해 상위 8개
  (`REASON_LIMIT`). 순위 배지 + 문구 + 발생 수 + 그중 타임아웃 비중. 스택 트레이스도 앞머리가 같으면
  하나로 묶인다
- **실패한 호출 표** (`FailedCallsTable`) — 호출 시각·결과·노드·모델·(키)·대기·사용자·질의·사유·TRACE_ID.
  컬럼 필터(결과/노드/모델/키 셀렉트, 사용자·질의·사유·TRACE_ID 텍스트) + 헤더 클릭 정렬(호출 시각/대기)
  + 페이징(25건). `QuestionsTable` 의 `qfilter-row`/`qft-*`/`qth-sort`/`qpager` 스타일 재사용.
  **서버가 내려준 최근 `ITEM_LIMIT`(200)건 안에서 좁히는 클라이언트 필터다**

## 조회 범위 = 기간 + 노드 + 모델 + 키 (전부 서버 필터)

기간은 공유 `TimeRangeProvider`. 노드/모델/키는 `DimCard` 행 클릭으로 걸리고(조합 가능) 상단
`to-scope` 칩으로 해제한다 — **노드별·모델별·키별 추이를 보는 수단이 이것이다**
(KPI·차트·분포·목록이 한꺼번에 좁혀진다).
