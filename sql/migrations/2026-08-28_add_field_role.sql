-- ============================================================================
-- [MIGRATION] TRX_USER_MAS : 권한에 FIELD(일반 사용자) 추가 — CHECK 제약 확장
--
--   배경 : 이 앱은 개발자가 4계층 메시지를 추적하려고 만들었지만, 실적 리포트 성격의
--          화면이 늘면서 **일반 사용자(비개발)**도 실적을 보고 싶어졌다. 일반 사용자에게는
--          레이어 JSON 원문 · 다른 사용자의 질의/사번 · 내부 에러 코드를 보이면 안 된다.
--          그래서 기존 3권한 아래에 **FIELD(일반 사용자)** 를 하나 더 둔다.
--
--            ADMIN(운영자) > BR(상위) > DEV(개발자) > FIELD(일반 사용자)
--
--          ⚠️ FIELD 는 서열만 낮은 게 아니라 **경로 허용 목록(allow-list)** 으로 판정한다.
--            ROUTE_RULES 는 "규칙에 없으면 통과"(fail-open)라 서열만 낮추면 앞으로
--            추가되는 화면이 자동으로 일반 사용자에게 열린다. 코드: src/lib/roles.ts
...
--          /accounts 에서 일반 사용자 계정을 만들 때 ORA-02290(CHECK 위반)으로 저장이 실패한다.
...
COMMENT ON COLUMN TRX_USER_MAS.ROLE_CD
    IS '권한 (ADMIN=운영자/BR=상위/DEV=개발자/FIELD=일반 사용자 — 실적 화면만)';
...
-- ============================================================================
-- [일반 사용자 계정 만들기]
--   화면(/accounts)에서 권한 "일반 사용자" 를 골라 만드는 것이 정상 경로다.
