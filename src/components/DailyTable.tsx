...
// ─────────────────────────────────────────────────────────────────────────────
// 일별 현황 표 — /insights 의 화면 표와 리포트 복사 텍스트가 공유한다.
//
// 원래 (지금은 삭제된) report/page.tsx 안에 있었는데 실적 화면이 같은 표를 그대로 써야 해서
// 컴포넌트로 뺐다. 지금은 화면(DailyTable)과 리포트 텍스트(lib/insightsReport.ts)가 같은
// `DailyRow[]` 를 받는다 — 두 벌로 두면 한쪽만 고쳐져 표와 복사본이 어긋난다.
