...
좌측 리스트 높이는 **우측 상세(사용자 대화 흐름 끝)에 맞춘다** — 스플릿 `align-items: stretch` +
`.rft-list { flex: 1 0 0; min-height: 420px }`. 고정 `max-height` 로 두면 상세보다 먼저 끊긴다.
1열(≤1080px)에서는 `max-height: 620px` 로 되돌린다.
