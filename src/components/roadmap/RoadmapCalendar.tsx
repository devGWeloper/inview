...
/**
 * 오픈 달력 — 이 화면의 전부다. (아래에 목록 표를 두지 않는다: "눈에 안 들어온다" 는 피드백으로
 * 통째로 뺐고, 항목의 설명·상태는 클릭하면 뜨는 팝업이 맡는다.)
 *
 *   년 보기(기본): **12달치 진짜 달력**을 깐다. 각 미니 달력 아래에 그 달의 이름 목록이 붙어
 *                  "며칠인가"(격자)와 "무엇인가"(목록)를 한 카드에서 같이 읽는다.
 *   월 보기      : 한 달을 크게. 칸이 넓어 이름이 그대로 들어간다.
 *
 * ⚠️ 달만 정해진 항목(`2026-10`)은 **어느 날짜 칸에도 넣지 않는다** — 1일 칸에 넣으면 없는
 *    확정일을 만들어 내는 셈이다. 목록에서 날짜 자리를 `–` 로 비우고, 월 보기에서는 격자 위
 *    '날짜 미정' 줄에 모은다. 시점 자체가 없는 항목은 달력 맨 아래 '일정 미정' 줄로 간다.
 */

export type CalMode = "month" | "year";

/** 한 칸에 여러 건이면 무엇으로 칠할지 — 급한 것이 이긴다. */
...
export function RoadmapCalendar({
  ...
  // 시점이 아예 없는 항목 — 달력 어디에도 자리가 없어 맨 아래 줄에 모은다.
...
function MiniMonth({
  ...
  // 6주 고정 — 12장의 카드에서 목록이 같은 높이에서 시작해야 격자로 읽힌다.
...
function MiniDay({
  ...
  // 이 달이 아닌 날은 격자 모양만 유지하고 비운다 — 미니 달력에서는 앞뒤 달 숫자가 잡음이다.
...
function Chip({
  ...
  selected: boolean;
  onPick: (id: string) => void;
  /** 칸이 날짜를 말해 주지 않는 곳(년 보기 목록)에서만 켠다 */
  showDate?: boolean;
  /** 월 보기의 넓은 칸용 */
  big?: boolean;
}) {
