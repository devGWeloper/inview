// ─────────────────────────────────────────────────────────────────────────────
// 대시보드/리포트 집계 (BIZ_AIACTIONTXN_HIS).
//
// 원래 /api/stats 라우트 안에 있던 계산을 lib 으로 끌어냈다 — 일반 사용자 실적 화면(/api/insights)이
// **같은 집계**를 쓰되 내려보내는 필드만 달라야 하기 때문이다. 집계 규칙이 두 벌이 되면
// 같은 기간인데 두 화면의 숫자가 갈린다. 계산은 여기 하나, 응답 모양은 라우트가 정한다.
//
// ※ server-only (Oracle 조회). 클라이언트에서 import 금지.
// ─────────────────────────────────────────────────────────────────────────────
...
/** 집계 조회 조건 (라우트가 쿼리스트링에서 파싱해 넘긴다). */
export interface StatsQuery {
  /** ISO(TZ 없음). 없으면 최근 24시간 */
  dateFrom?: string;
  dateTo?: string;
  /** 트레이스 단위 필터 (행 단위로 걸면 다른 레이어 행이 빠져 트레이스가 깨진다) */
  userId?: string;
  actionTyp?: string;
  /** 이 에러 코드를 가진 트레이스를 집계에서 통째로 제외 */
  excludeErrCds?: string[];
}

/** 집계 결과 + 진단용 부가 정보 (라우트 로깅에 쓴다). */
...
/**
 * 기간 내 트레이스를 읽어 대시보드/리포트용 집계를 만든다.
 * ⚠️ 호출 전에 반드시 인가(requireBiz)를 통과시킬 것 — 이 함수는 권한을 보지 않는다.
 */
