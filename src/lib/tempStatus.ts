...
//
// 임시 규칙: 에러 코드가 없는 미완료(pending) 트레이스는 GAIA 레이어의 RESP 메시지로 판정한다.
//   - GAIA RESP 메시지에 "Seasoning 실패" 문구 포함  → fail
//   - 그 외                                          → ok (성공으로 간주)
//
...
/** GAIA RESP 메시지에 Seasoning 실패 문구가 있는지 */
export function hasSeasoningFailure(rows: TraceRow[]): boolean {
  return rows.some(
    (r) => r.layer === "GAIA" && !!r.respMsgCtn && r.respMsgCtn.includes(SEASONING_FAIL_PHRASE)
  );
}

/** pending 대체 판정: Seasoning 실패면 fail, 없으면 ok */
export function classifyPendingByGaiaResp(rows: TraceRow[]): TraceStatus {
