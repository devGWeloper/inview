...
//
// 화면: 좌측 도넛(비중) + 우측 표(세부값). 도넛은 recharts 를 쓰지 않고 직접 그린다 —
// recharts v3 의 Pie 에는 제어형 activeIndex 가 없어 표 hover ↔ 도넛 하이라이트 동기화가 안 된다.
...
function ShareDonut({
  ...
  // 12시 방향에서 시계방향. 한 조각뿐이면 갭 없이 꽉 찬 링.
...
export function LayerBudget({ stats }: { stats: StatsResponse }) {
  ...
  // 도넛 중앙 — hover 중엔 그 레이어, 아니면 전체 요약.
