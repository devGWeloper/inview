...
좌측 리스트 높이는 **우측 상세(사용자 대화 흐름 끝)에 동적으로 맞추고** 넘치면 자체 스크롤한다 —
스플릿 `align-items: stretch` + `.rft-list { flex: 1 1 auto; height: 0; min-height: 480px }`.
`height: 0` 은 리스트 내용이 그리드 행 높이에 기여하지 않게 하는 것 — 행 높이는 우측 상세만 정한다.
그래서 대화 흐름이 길면 리스트도 그만큼 길어진다. 1열(≤1080px)에서는 `max-height: 800px`.
...
기간·상태 아래 `rft-errbar` — 기간 내 에러코드 분포 칩. 칩을 끄면 그 코드는 목록에서 빠진다
(`ERROR_LLM` 처럼 볼 필요 없는 코드를 치우는 용도). `ERR_CD IS NULL` 은 `NO_ERR_CD`(`__NONE__`)
센티넬로 한 칩(`라우팅 실패`) 이 된다.

**제외는 클라이언트가 아니라 서버(`excludeErrCd`, 콤마 구분)에서 건다** — 클라 필터로 하면
`limit=400` 상위가 그 코드로 다 차서 정작 볼 요청이 잘려 나간다. 그래서 KPI·상태칩 카운트도
제외 후 기준이다.

칩 분포(`errCodes`)는 **제외를 걸기 전 WHERE** 로 따로 GROUP BY 한다. 안 그러면 한 번 끈 코드가
분포에서 사라져 되살릴 수 없다. 분포 쿼리 실패 시 조회된 목록으로 폴백(무해).

끈 코드는 `localStorage["tracex.rft.hiddenErrCds"]` 에 남아 다음 방문에도 유지된다.
