...
- **단일 소스는 `TimeRangeProvider`** — `AppChrome` 의 `AgentScopeProvider` 안쪽에 마운트되고
  ...
  ⚠️ 분 단위 프리셋(1M/5M/…)을 여기 넣지 말 것 — 그건 틱 뷰의 창 길이가 맡는다(한 번 넣었다 물렀다).
...
- **틱(1TICK) 창은 `TimeRangeProvider` 가 아니라 `TickProvider` 가 쥔다** — 격자(초/분)도
  응답 형태도 달라 같은 상태에 담을 수 없다. 다만 성격이 같아 **Dashboard/Tokens/Timeout 셋이
  공유**한다(`localStorage["tracex.tick"]`). 자세한 건 아래 "틱(1TICK)" 절.
- **Dashboard/Report/Insights 는 공유 대상이 아니다** — 기간 개념이 다르다(주 단위 이동, 월~월 경계 등).

### 틱(1TICK) — 롤링 60초 모니터 (Dashboard · Tokens · Timeout 공용)

사내 LLM 제약이 **TPM/RPM(분당 토큰·호출)** 이라 초과 여부를 화면에서 봐야 하는데, 기존 추이는
5분/1시간/1일 격자라 분당 판정이 불가능했다. 그래서 Tokens 탭에 1TICK 을 만들었고, 같은 형식이
다른 대시보드에도 필요해져 **뷰(보기 모드)로 승격**했다.

- ⚠️ **정각 분 버킷은 판정 기준이 못 된다.** 제약은 "임의의 연속 60초" 기준이라, 12:01:13~12:02:12 에
  몰린 버스트는 정각 격자에선 두 칸으로 쪼개져 어느 칸도 한도를 안 넘는 것처럼 보인다(실제로는 초과).
  그래서 **초 단위 SQL 집계 위에서 슬라이딩 60초 윈도우의 최대값**을 구한다. 윈도우 시작을 "값이 있던 초"
  로만 잡아도 실수 t 전체의 최대와 같으므로(합은 구간별 상수, 어떤 시작점의 창이든 그 안 첫 이벤트 시각에서
  시작하는 창이 같은 이벤트를 모두 포함) **근사가 아니라 정확한 최대**다.
- ⚠️ **롤업은 순수 함수 `rollupTick()`(`src/lib/tickStats.ts`) 하나다** — LLM 소스와 BIZ 소스가 같이 쓴다.
  슬라이딩 60초의 정의가 소스마다 갈리면 같은 사건에서 다른 수치가 나온다. 경계 조건(정확히 60초 간격은
  창 밖, 빈 입력, 같은 초 다건) 검증도 여기 하나로 끝난다.

**⚠️ 지표는 화면마다 다르고, 이름은 서버가 모른다.** 서버는 이름 없는 **A/B 두 슬롯**(`TickMinute.rollA/rollB`)만
채우고, "A 가 무엇인가"(라벨·단위·한도)는 화면이 `TickMetricDef` 로 정한다. 한도는 프로필과 `config.yml` 을
병합해 `/api/agents` 가 만드는 값이라 **서버 집계가 알 수 없기 때문**이다 — 그 병합을 tick 집계에 복제하지 않는다.
...
| `/dashboard` | 요청/분 | 실패/분 | 없음(0) | `BIZ_AIACTIONTXN_HIS` 진입 레이어 | `/api/stats/tick` |

- ⚠️ **Timeout 에 TPM/RPM 을 복제하지 않는다** — 숫자가 Tokens 탭과 글자 하나까지 같아져 화면이 있을
  이유가 없어진다. 같은 테이블에서 `SQL_ERR_PRED`/`SQL_TIMEOUT_PRED` 로 세어 **다른 지표**를 만든다
  (`view=failure`). ⚠️ `STAT_CD`/`ERR_CTN` 컬럼이 없으면 `statusAvailable=false` 로 내려 화면이
  "측정 불가" 로 밝힌다 — 0 건으로 보이면 "문제 없음" 으로 오독된다. 반대로 **usage 뷰에서는 이 필드를
  안 내린다**(사용량 조회엔 STAT_CD 가 필요 없어 헛경고가 된다).
- ⚠️ **BIZ 집계는 진입 레이어(`LAYER_ORDER[0]`) 하나만 읽는다** (`src/lib/bizTickStats.ts`). 사용자 요청
  1건 = 진입 레이어 수신 1건이라 "분당 몇 건" 은 여기서만 정의된다. 하위 레이어를 같이 세면 멀티콜
  (GAIA→MCP 2회) 때문에 요청 수가 부풀어 오른다. 실패 판정은 화면 나머지와 **같은 규칙**이다
  (`ERR_CD` 또는 TEMP 액션 실패 문구 — 여기만 `ERR_CD` 로 좁히면 대시보드 KPI 와 틱 뷰의 실패 수가 갈린다).
  ⚠️ `ACTION_TYP`/`FAC_ID` 는 **담기지 않는다** — 다른 레이어가 기록하는 값이라 진입 레이어 행에는 없다.
  그래서 대시보드의 ACTION_TYP 셀렉트는 틱 뷰에서 감춘다(걸어도 효과가 없는데 걸린 것처럼 보이면
  "이 액션은 요청이 없다" 로 오독된다).
- ⚠️ 라우트가 `/api/traces/tick` 이 **아닌** 이유: 그쪽엔 `[traceId]` 동적 세그먼트가 있어 "tick" 이라는
  이름의 트레이스와 경로가 겹친다. 대시보드 데이터 라우트(`/api/stats`)의 형제로 둔다.
- ⚠️ `/api/tokens/tick` 의 **모르는 `view` 는 400 이다** — 조용히 기본으로 떨구면 실패 수를 사용량으로
  읽어 정반대로 오독된다(에이전트 id 를 400 으로 막는 것과 같은 이유).

**UI — 보기 토글 `집계 | ● 틱` (`src/components/tick/`)**

- ⚠️ **워딩은 증권 차트 탭(`틱 · 1분 · 일 · 주 · 월`)에서 가져왔다.** 처음엔 "실시간" 이었는데 물렀다 —
  이 뷰는 직접 설정으로 과거 구간도 보고 자동 갱신도 기본이 꺼져 있어 최신성을 주장하는 말이 맞지 않았다.
  실제로 갈리는 건 **집계 단위**다. (엄밀히는 체결 단위 '틱' 이 아니라 롤링 60초지만, 팀이 이미
  1TICK 이라 부르므로 통하는 말을 택했다. "틱 모니터" 도 후보였으나 반려됐다.)

- ⚠️ **진입은 머리말 우상단의 `ViewToggle` 이다** — 제목과 같은 줄(`.dash-head-row`),
  조회 줄(`.dash-filter`) **밖**. 자리가 두 번 옮겨졌고 이유가 각각 다르다:
  ① 원래는 기간 프리셋 줄 **안의** `1TICK` 버튼이었다 — 그 줄의 다른 버튼은 전부 "조회 기간" 인데
  이것만 화면을 통째로 바꿔서, 기간을 고른 줄 알고 눌렀다가 화면이 달라지는 일이 있었다.
  ② 그래서 줄 밖 첫 칸으로 뺐더니, 조회 줄이 한 줄에 안 들어가는 폭에서 **이 토글만 윗줄로
  튀어 올라** 줄이 깨져 보였다(피드백: "줄바꿈돼서 다 깨진다"). 조회 조건과 **아예 다른 줄**에
  두면 조회 줄이 몇 줄로 접히든 토글 자리는 안 흔들린다.
  따라서 세 화면의 머리말은 `.dash-header.stacked` 로 **두 줄**이다 —
  1줄 = 제목 + 토글(우상단), 2줄 = 조회 조건 전폭. ⚠️ 토글을 조회 줄로 되돌리지 말 것.
  (`.dash-header` 자체는 `/insights` 도 쓰므로 건드리지 않고 `.stacked` 로만 갈린다.)
- ⚠️ **버튼 두 개가 아니라 thumb 이 미끄러지는 스위치다** (`.view-toggle-thumb`).
  칸은 `inline-grid` + `grid-auto-columns: 1fr` 로 **강제 균등**이라 글자 수(집계 2자 / 틱 1자)와
  무관하게 thumb(`width: calc(50% - 3px)`, `translateX(100%)`)이 정확히 반 칸을 덮는다.
  칸 폭을 내용에 맡기면(`inline-flex`) thumb 폭이 상태마다 달라져 미끄러지지 않는다.
- ⚠️ **기간 프리셋 줄 자리는 뷰에 따라 통째로 교체된다** — 틱이면 그 자리에 `TickPresets`
  (창 길이 + 직접 설정)가 들어간다. 두 줄을 같이 띄우면 "기간을 골랐는데 왜 안 바뀌지" 가 다시 생긴다.
- ⚠️ **토글해도 화면이 흔들리지 않게 두 가지를 맞춰 뒀다** (피드백: "토글 바꿀 때마다 눈이 피곤하다"):
  ① 교체되는 자리를 `.preset-slot` 으로 감싸 **넓은 쪽(틱 8개) 폭을 미리 확보**한다 — 안 그러면
  버튼 수 차이(집계 6개)만큼 뒤따르는 USER/NODE/MODEL 이 좌우로 밀린다.
  ② 동작 버튼은 두 보기가 **같은 자리**를 쓴다 — 그래서 툴바가 `TickPresets`(프리셋 자리) 와
  `TickActions`(`조회` 버튼 자리, 자동 갱신 + 새로고침) **두 조각**으로 나뉘어 있다. 한 덩어리로
  합치지 말 것.
- ⚠️ **조회 구간은 토글할 때 서로 물려준다** (`src/components/tick/rangeSync.ts`, 순수 함수).
  두 뷰의 후보가 겹치지 않아(집계 1H~30D / 틱 1~180분) 정확히 같을 수는 없지만, 무관한 구간으로
  튀면 매번 기간을 다시 골라야 한다. 집계→틱은 **그 길이 이하로 가장 큰 창**(6H 이상은 전부 180분),
  틱→집계는 **그 창을 덮는 가장 짧은 프리셋**. 직접 설정은 **24시간 이하일 때만** 그대로 넘긴다 —
  더 길면 서버가 뒤쪽 24시간만 남기고 잘라 토글 한 번에 경고 배너부터 보게 된다.
  ⚠️ 억지로 더 맞추려고 집계에 '5분' 을, 틱에 '7일' 을 넣지 말 것 — 각각 칸 1개짜리 추이와
  상한에 잘리는 창이 된다. **딱 안 맞는 게 의도**다.
- **`● 틱` 의 점은 자동 갱신이 도는 동안에만 깜빡인다**(`.live-dot.pulse`) — 지금 살아 있는 값인지
  멈춘 값인지를 글자 없이 알리는 표시다. 문구로 설명하지 말 것.
- **창 길이는 `TICK_WINDOWS = [1, 5, 10, 15, 30, 60, 180]`(분).** 자동 갱신 주기는 창에 맞춘다
  (`tickRefreshMs`: 10분 이하 → 10초, 그 위 → 30초) — 1분 창을 30초마다 갱신하면 반쯤 죽어 보이고,
  3시간 창을 10초마다 부르면 낭비다. ⚠️ 1분 창은 롤링 값이 1~2칸뿐이라 **게이지 위주**로 읽는 용도다.
- **공유 범위**: 창 길이·모드·직접 설정 구간·자동 여부는 `TickProvider` 가 세 화면과 **공유**하고,
  **뷰 on/off 만 화면별**이다(`useTickView(key)` → `localStorage["tracex.tick.view.<key>"]`).
  대시보드를 틱으로 띄워 두고 Tokens 는 집계로 보는 조합이 정상이기 때문.
  ⚠️ `TickProvider.persist` 는 **`selRef` 를 동기적으로 먼저 갱신**한다 — 화면이 창을 고른 직후
  곧바로 `resolve()` 로 조회하는데, `setSel` 만 하면 다음 렌더 전까지 이전 창으로 조회하게 된다.
- ⚠️ 조회 시각은 `toLocalSec()`(초 정밀)로 만든다 — 분 정밀 + `":00"` 을 쓰면 현재 분이 통째로 잘려
  방금 난 버스트가 안 잡힌다.
- **조회 구간은 두 모드**다. `live` = 창 길이만큼 "지금까지"(창이 계속 앞으로 밀린다).
  `custom` = `직접 설정` 으로 찍은 고정 구간(과거 이력을 보는 경로).
  ⚠️ `custom` 에서는 **자동 갱신을 잠근다**(체크박스 disabled + effect 가드) — 고정 구간을 다시 부를
  이유가 없을 뿐 아니라, 라이브 갱신은 매번 '지금' 으로 창을 다시 잡아 지정한 구간을 덮어쓴다.
  ⚠️ 서버는 24시간(`TICK_MAX_MINUTES`)을 넘는 구간을 **뒤쪽(최신)만 남기고** 자르므로, 요청한 `from`
  보다 응답 `range.from` 이 뒤면 `clamped` 로 화면이 밝힌다 — 안 밝히면 잘려나간 앞 구간이
  "그 시간엔 호출이 없었다" 로 오독된다.

**본문 (`TickMonitor.tsx` / 차트 `TickMonitorChart.tsx`, `tick-*`)** — 위에서 아래로 **세 가지만** 답한다:
① **게이지 2장**(A/B: 값 + 한도가 있으면 대비 막대·%) ② **추이 차트** ③ **순간 목록**.

- ⚠️ **정각 분 합계(`fixed*`)는 그리지 않는다.** 처음엔 막대(정각 분)+선(롤링)을 겹쳐 그렸는데
  "막대랑 선이 뭐가 다르냐"는 질문이 바로 나왔다 — 판정값과 비판정값을 나란히 두면 어느 게 기준인지
  헷갈린다. 차트는 롤링 60초 값 하나만 Area 로 그리고 한도는 점선 하나. 초과한 분만 점을 찍는다.
  (`fixed*` 는 응답에 남아 있다 — 같은 쿼리에서 공짜로 나오고 진단에 쓸 수 있어서.)
- ⚠️ **A/B 전환은 게이지 카드 클릭**이다. 별도 토글 + KPI 카드를 따로 두니 같은 수치가 두 번 나와
  화면이 복잡해졌다. 게이지가 곧 선택지 역할을 한다.
- **순간 목록은 한도 유무로 갈린다**: 한도가 있으면 **"초과한 순간"**(연속 초과 분을 구간으로 병합),
  없으면 **"가장 몰린 순간"**(값 상위 TOP 5). ⚠️ 후자는 같은 버스트의 이웃 분이 목록을 채우지 않도록
  이미 고른 순간과 60초 안에서 겹치는 후보를 건너뛴다. ⚠️ 그리고 **빨간색을 쓰지 않는다**
  (`.tick-seg-item.neutral`) — 초과가 아닌 정상값을 사고로 읽게 된다.
- 행을 열면 그 피크 60초의 원본 행이 나온다 — LLM 소스는 호출 표(`TickCall`), BIZ 소스는 요청 표
  (`TickTrace`). ⚠️ 하나의 범용 표로 합치지 않았다: 열이 서로 겹치지 않아(IN/OUT/TOTAL vs 사용자/에러코드)
  억지로 합치면 양쪽 다 빈 칸투성이가 된다.
- ⚠️ 시각은 **`windowLabel()` 로 "09:16:30 ~ 09:17:30"** 처럼 끝 시각을 직접 적는다. `+60s` 표기는
  한 번 더 계산해야 해서 폐기했다. 차트 툴팁은 이 **구간을 제목으로** 올려 라벨 없이 보여준다 —
  "가장 몰린 60초" 같은 설명 라벨은 화면에서 뺐다(직관적이지 않다는 피드백).
  화면 문구는 값이 무엇인지 설명하려 들지 말고 숫자·시각·%만 보여줄 것. 계산 방식 설명은 여기에만 둔다.
- **조회 툴바는 `TickMonitor` 안에 없다** — 페이지 헤더의 `TickPresets`/`TickActions` 가 담당한다.
  두 줄로 나뉘어 있으면 어느 쪽이 지금 조회 조건인지 헷갈린다.
- 기존 USER/NODE/MODEL 필터는 그대로 적용된다(각 페이지가 현재 뷰에 맞는 쪽을 다시 조회).
- **틱 뷰가 아닐 때 세 화면은 기존 경로 그대로다** — 기존 화면 로직은 바뀌지 않았다.
...
- **틱 뷰**: 상단 `집계 | 틱` 토글로 **분당 타임아웃/실패**(롤링 60초)를 본다 — 위 "틱(1TICK)" 절 참고.
  ⚠️ Tokens 탭의 TPM/RPM 을 복제하지 않는다(같은 숫자가 두 번 나온다).
- ⚠️ **두 계열의 색은 명도까지 벌려 둔다** — 예전엔 타임아웃 `#b42318` / 기타 오류 `#c2410c` 라
  색상·명도가 모두 가까워 적층 면적에서 경계가 안 보였다("구분이 안 간다"). 지금은 타임아웃
  `#b42318`(진한 빨강) / **LLM 오류 `#d97706`(앰버, `--llm-err`)**. 차트는 JS 상수
  (`TimeoutTrendChart.SERIES_COLOR`), 막대·배지는 CSS 변수를 쓰므로 **두 값을 같이 고칠 것**.
- **문구는 "기타 오류" → "LLM 오류"** 로 바꿨다(추이 범례·분포 막대·목록 필터·상태 배지).
  ⚠️ 타임아웃도 넓게 보면 LLM 오류지만, 이 화면에서 둘은 **배타적인 두 계열**이다
...
- **읽기/쓰기**: `src/lib/eventFabs.ts` → `GET/PUT /api/event-fabs`. 읽기는 lazy-`oracledb`-swallow 패턴으로 미구성/미생성 시 `available=false + reason` 을 내려 화면이 안내하고 저장을 막는다. **저장은 전량 교체**(DELETE 후 INSERT, 한 트랜잭션, 실패 시 rollback + throw) — 앱이 이 테이블의 마스터. FAB 0개 행은 "미등록" 과 구분이 안 돼 저장 거부(행 삭제를 강제). ⚠️ **조회는 BR, 저장(PUT)은 ADMIN** — `requireBiz("ADMIN")`.
- **화면**: `/event-fabs` (클라이언트, **미들웨어 인가 BR 이상** 뒤. BR 은 **열람 전용** — `canEdit`(ADMIN)이 false 면 도트/행 액션/저장이 잠기고 툴바에 "열람 전용" 배지가 뜬다. 권위는 서버 PUT). **권한 매트릭스 콘솔** 스타일(`fm-*`): 컴팩트 툴바(작은 타이틀 + 이벤트 검색 + "+ 이벤트"/저장) 아래 이벤트(행)×FAB(열) 매트릭스 — 스티키 헤더 + 패널 내부 스크롤이라 이벤트 100개 스케일을 전제. 셀 = 토글 도트(켜면 액센트 채움+체크 팝), **열 헤더 클릭 = 보이는 행 대상 열 일괄 토글**, 행 액션(행 전체 토글/삭제)은 hover 시에만 노출, 이벤트명은 borderless 인라인 입력(`/api/action-types` datalist). 저장 버튼은 **dirty(스냅샷 비교) 일 때만 활성** + 흰 점 표시, FAB 0개 행은 "팹 없음" 배지. 안내문은 하단 풋노트 한 줄로 축약. 진입은 `/admin` 헤더의 "이벤트-FAB 매핑" 버튼.
...
- **API**: `POST /api/auth/login`·`logout`, `GET /api/auth/me`(비로그인 200+`{user:null}`), `POST /api/auth/change-password`(본인). 계정관리(**ADMIN** + 위 잔여 방어 가드): `GET/POST /api/accounts`, `PUT/DELETE /api/accounts/[userId]`, `POST /api/accounts/[userId]/reset-password`. 서버 방어는 `src/lib/auth/current.ts` `requireRole(min)`.
...
- ⚠️ **`/insights` 는 반대로 위쪽도 좁힌다 — 일반 사용자 + BR 이상이다** (`canViewInsights(role)`).
  **DEV 만 못 본다** — 개발자는 Dashboard 로 같은 수치를 더 자세히 보고, BR·ADMIN 은 "일반 사용자에게
  무엇이 보이는가" 를 같은 화면으로 확인해야 한다. 그래서 이 판정은 `ROUTE_RULES`(서열, fail-open)에
  실을 수 없다 — 규칙에 없으면 통과라 DEV 까지 열리기 때문이다. `canAccessPath()` 가 다른 규칙보다
  **먼저** 이 판정을 쓴다.
...
- **집계는 공유, 응답은 분리** — 대시보드 집계 계산을 라우트에서 끌어내 `src/lib/stats.ts`
  ...
  새로 새고, 담는 방식이면 안 샌다. 빠지는 것: `topUsers`(사번) · `layers`/`selfTime`(내부 구조) ·
  `excludeErrCds`. 남는 것: 상태 합계 · 성공률 · 평균 응답 · **사용자 '수'** · 버킷 · 일별 ·
  기능(ACTION_TYP)별 · FAC별 · **주요 실패 원인** · 프로필 공개 항목 · FTE.
- ⚠️ **주요 실패 원인(`topErrors`)은 코드가 아니라 '사유' 로 내린다** (`InsightsError`
  = `{code, label, count, described}`, `toInsightsErrors()`). 이 화면은 개발자가 아닌 사람이 보므로
  서버가 **설명을 붙여** 내리고 화면은 설명을 앞세운다. 설명 출처는 둘 — ① `TRX_ERRMSG_COD` 마스터
  (`loadErrorCodeMap()`) ② TEMP 가상 코드 라벨(`ACTION_FAIL_LABELS`, `tempStatus.ts`). 둘 다 없으면
  `label = code` + `described=false` 로 내려 화면이 코드를 두 번 그리지 않는다.
  ⚠️ **일반 사용자는 `/api/error-codes` 를 못 부른다**(FIELD 허용 목록 밖) — 그래서 설명 조회를
  `/api/insights` 안에 넣었다. 이 화면의 데이터 소스는 끝까지 이 라우트 **하나**여야 한다.
  ⚠️ 막대는 **1위 대비 상대 길이**이고 %를 쓰지 않는다 — `topErrors` 의 분모가 실제 `ERR_CD`(행 단위)와
...
- **화면 `/insights`** (`src/app/insights/page.tsx`, `ins-*`): 기간 선택은 세 갈래다 —
  ...
  **두 단**이 이어진다 — ① 업무 실적: KPI 6(처리 건수·성공률·실패·평균 응답 속도·사용 인원·누적 절감
  FTE) + 처리 추이(전폭) + **일별 현황 표**(2일 이상 조회일 때) + [기능별 실적 | **주요 실패 원인**] 2열
  ("무엇을 처리했나 / 무엇이 안 됐나" 한 쌍 — 같은 형태의 막대 목록이라 나란히 둔다)
  + [절감 효과 추이 | **FAB별 실적**] 2열,
  ...
  ⚠️ 카드를 혼자 한 줄에 두면 **가로만 길고 안이 비어 보인다** — 조밀하지 않은 카드는
  `.ins-grid-2`(auto-fit minmax 380px, 좁으면 1열)로 짝지어 둘 것. **절감 효과 추이(FteChart)가
  이 규칙의 출처다**: 두 번 전폭으로 뒀다가 두 번 다 "보기 흉하다" 는 지적을 받고 되돌렸다.
  카드를 추가·삭제해 짝이 홀수가 되면 **남는 한 장을 전폭으로 늘리지 말고** 짝을 찾을 것
  (FAB별 실적이 그렇게 들어왔다 — `byFac` 는 원래 응답에 있었는데 화면에서 안 쓰고 있었다).
  ⚠️ **FAB별의 `(none)`(=MCP 미도달)은 '미상' 으로 표기하고 흐리게** 둔다. 빼 버리면 합이
  총 처리 건수와 안 맞아 "왜 숫자가 다르냐" 가 된다 (기능별의 `라우팅 실패` 와 같은 취급).
...
- **BR·ADMIN 은 일반 사용자가 보는 것을 같은 화면으로 본다** — 별도 미리보기를 만들면 두 화면이
...
- ⚠️ **일반 사용자 계정의 소속은 기본 에이전트여야 한다** — `/insights` 는 BIZ_AIACTIONTXN_HIS 집계라
  ...
  ⚠️ 그래서 **계정 저장 시점에 막는다** — `roles.ts` 의 `scopeErrorForRole(role, scope, defaultAgent)` 가
  단일 소스이고, `/api/accounts` 의 POST 와 `[userId]` PUT 이 호출해 400 으로 거절한다
  (PUT 은 요청만이 아니라 **최종 상태**로 판정한다 — 권한만 FIELD 로 바꾸는 경우와
  FIELD 계정의 소속만 옮기는 경우를 둘 다 걸러야 하기 때문). 화면(`/accounts`)은 권한을
  일반 사용자로 고르면 에이전트 셀렉트를 기본 에이전트로 고정·잠그지만 그건 UX 다.
  예전엔 검증이 없어 저장은 되고 로그인하면 홈부터 403 인 — 만든 사람도 받는 사람도
  이유를 모르는 — 계정이 만들어졌다.

### 공사장 — `/wip` (아직 안 연 화면 모음, ADMIN 전용)

만들다 만 화면이 상단 탭이나 유저 메뉴에 하나씩 붙으면 정식 화면과 섞여 "이건 써도 되는 건가" 를
매번 되묻게 된다. 그래서 **아직 운영에 안 내보낸 화면은 전부 `/wip` 뒤**에 두고, 상단바에는
점선 칩 하나(`.wip-entry`, ADMIN 에게만 렌더)만 남긴다. 예전에 상단바에 따로 붙어 있던
"디자인 시안" 버튼이 이 안으로 들어갔다.

- **항목 추가는 `src/app/wip/page.tsx` 의 `SITES` 배열 한 줄.** 정식 오픈 = 거기서 지우고
  `TabNav`(또는 `UserMenu`)로 옮긴다.
- ⚠️ **이 목록은 표시일 뿐 접근 제어가 아니다** — 각 항목의 실제 차단은 자기 경로의
  `ROUTE_RULES`/API 가드가 한다. 여기서 지운다고 그 화면이 잠기지 않는다.
- 접근은 `ROUTE_RULES` 의 `{ prefix: "/wip", min: "ADMIN" }` + 페이지의 `requireRole("ADMIN")`.
- 현재 들어 있는 것: **Action 오픈 로드맵**(아래) · **레이아웃 개편 시안**(`/design-preview.html`).

### Action 오픈 로드맵 — `/roadmap` (⚠️ DB 아님 · 파일 저장 · 지금은 공사장 안)

Action 이 계속 늘어나는데 "언제 무엇이 열렸고 앞으로 무엇을 열 것인가" 를 모아 둔 곳이 없어
매번 사람에게 물어야 했다. **DB 연동이 없는 계획표**다 — 운영자가 직접 적고 나머지 전원이 읽는다.

- **화면은 달력 하나뿐이다.** 통계 줄도, 아래 목록 표도 없다 — 둘 다 만들었다가 뺐다
  ("통계는 빼자", "밑에 있는 정보 눈에 안 들어온다, 과감히 지우고 달력에만 집중"). ⚠️ 다시 붙이지 말 것.
  항목의 **설명·상태·정확한 날짜는 클릭하면 뜨는 팝업**(`MilestoneDialog`)이 맡는다.
  그래서 그 팝업은 **수정 권한이 없는 사람에게도 열린다**(`readOnly`) — 표를 없앤 뒤로
  그 정보를 볼 수 있는 유일한 자리이기 때문이다. 팝업에서 필드를 빼면 화면 어디에도 안 남는다.
- **달력은 두 배율이다** (`CalMode`, 툴바의 `년 | 월` 토글).
  **년 보기가 기본이고 12달치 진짜 달력을 깐다** — 미니 달력(며칠인가) + 그 아래 이름 목록
  (무엇인가)을 한 카드에서 같이 읽는다. 요약 카드(이름 목록만)였던 적이 있으나
  "차라리 12달치 달력을 다 보여 달라, 공간 차지해도 된다" 는 요청으로 바뀌었다.
  달 제목을 누르면 그 달의 월 보기로 내려간다.
  `‹ ›` 이동 단위는 **보기 단위를 따른다**(년 보기에서 한 달씩 움직이면 화면이 안 바뀐다).
  ⚠️ 미니 달력은 **`buildMonthGrid(..., 6)` 으로 6주 고정**이다 — 달마다 주 수가 다르면
  카드마다 아래 목록이 시작하는 높이가 어긋나 12장이 격자로 안 읽힌다. 큰 월 보기는 반대로
  필요한 만큼만(5~6주) 그린다. 빈 달 카드도 자리를 지킨다(빼면 달이 밀린다).
  ⚠️ 미니 달력 칸은 **상태 색으로 채우기만** 하고 이름은 안 쓴다(들어갈 폭이 없다) —
  이름은 바로 아래 목록이 맡는다. 한 날에 여러 건이면 `dominant()` 규칙
  (지연 > 개발중 > 계획 > 완료 > 보류)으로 칠하고 모서리에 건수를 띄운다.
- ⚠️ **달력 칸을 세로로 아끼지 말 것** — 월 보기 칸을 74px 로 뒀다가 "위아래로 짜부돼 있다" 는
  지적을 받고 118px 로 늘렸다. 달력이 이 화면의 전부라 세로 공간을 아낄 이유가 없다.
- **시점이 아예 없는 항목은 달력 맨 아래 '일정 미정' 줄**에 모인다(`.rm-cal-loose.bottom`).
  표가 있던 시절엔 표 맨 아래 그룹이었다 — 표를 지웠으니 이 줄이 없으면 그 항목이 화면에서 사라진다.
- **저장은 `data/roadmap.json` 하나** (`src/lib/roadmap.ts`, `profile.ts` 와 같은 fs 패턴).
  ⚠️ **에이전트별로 나누지 않는다** — 앱 전체에 1벌이고 쓰기 권한도 전역 ADMIN 하나다.
  `normalizeRoadmap()` 이 부분/깨진 입력을 항상 완전한 객체로 보정한다(이름 빈 행 버림,
  `id` 중복·경로문자 제거, 알 수 없는 status → `planned`, 항목 300개 상한).
  `data/` 는 gitignore 라 **배포 직후에는 빈 상태**로 시작한다(화면이 "첫 Action 추가" 로 안내).
- **권한 = 읽기 전원 / 쓰기 전역 ADMIN.**
  읽기는 일반 사용자까지 — `roles.ts` `FIELD_ALLOW_PREFIXES` 에 `/roadmap`·`/api/roadmap` 추가
  (`ROUTE_RULES` 에는 규칙을 두지 않는다 = 인증만 되면 통과). ⚠️ 그래서 **사번·원문·에러 코드 같은
  내부 정보를 이 화면에 얹지 말 것** — FIELD 화면의 원칙은 `/insights` 와 같다.
  쓰기는 `src/lib/auth/current.ts` 의 **`requireGlobalAdmin()`**(신규). ⚠️ `requireAgentAdmin` 을
  쓰면 안 된다 — 그쪽은 "그 에이전트의 ADMIN" 도 통과시켜, 에이전트 하나에 매인 운영자가
  앱 공용 문서를 고치게 된다. 화면의 `canEdit`(`role==="ADMIN" && user.global`)은 UX 이고
  권위는 `PUT /api/roadmap` 이다.
  ⚠️ **BIZ 경로가 아니다** — `isBizPath`/`requireBiz` 를 붙이지 않는다. BIZ_AIACTIONTXN_HIS 도
  TRX_TOKEN_DET 도 아닌 파일이라 에이전트 범위와 무관하고, 다른 팀 계정이 못 볼 이유가 없다.
- **편집은 팝업 하나(`MilestoneDialog`)로만 한다.** 달력의 칩이나 채워진 날짜 칸 클릭 = 열기,
  머리말의 `+ Action 추가` = 생성. 저장하면 **그때마다 전체 목록을 PUT** 한다 — 화면이
  '저장 안 한 변경' 상태를 들고 있지 않게 하기 위해서다(한때 화면 아래에 편집 목록 + 저장 버튼을
  뒀다가 "밑에 추가 영역 만든 건 별로다" 로 폐기).
  ⚠️ 시점은 **네이티브 `date`/`month` 입력**으로 받는다(세그먼트 `날짜 | 월만 | 미정`).
  자유 입력이면 형식 오류를 사용자가 감당해야 하고 브라우저 달력 피커도 못 쓴다.
- **시점 정밀도는 날짜(`2026-09-24`)와 월(`2026-09`) 둘뿐이다.** 해석은
  `src/lib/roadmapTime.ts`(**순수 함수만**, 서버·클라이언트 공용)의 `parseWhen()`.
  ⚠️ **분기·연도(`2026-Q4`)는 일부러 뺐다** — Action 은 실제로 한 달 안에 개발해서 여는 단위라
  분기는 너무 뭉툭하고, 무엇보다 **달력에 놓을 자리가 없다**. 아직 모르면 비워 두면 되고
  (표 맨 아래 "일정 미정"), 달만 정해졌으면 월로 적으면 된다.
  ⚠️ 달만 정해진 항목을 **1일 칸에 넣지 말 것** — 없는 확정일을 만들어 내는 셈이다.
...
- **지연(`overdue`)은 저장하지 않는다** — `isOverdue()` 가 목표 구간 끝과 오늘을 비교해 파생시킨다.
  운영자가 상태를 손으로 갱신하지 않아도 목표일이 지난 항목이 스스로 드러난다. 완료·보류는 대상 아님.
- **첫 화면 달은 '이번 달' 이 아니다** (`initialMonth`) — 오늘 이후 가장 가까운 항목의 달,
  앞으로 아무것도 없으면 마지막 항목의 달. 이번 달이 비어 있는데 이번 달을 열면 빈 화면만 보인다.
- ⚠️ **아직 상단 탭에 없다** — 진입은 공사장(`/wip`)뿐이다(위 참고). 열람 권한(FIELD 포함)만
  미리 열어 둔 상태라, **정식 오픈은 `TabNav` 의 `ANALYSIS_TABS` 에 한 줄 추가**하는 것으로 끝난다
  (`{ href: "/roadmap", label: "로드맵", … }`, `agentScoped` 없이 = 비기본 에이전트에선 자동으로 숨음).
  같이 할 일: `/wip` 의 `SITES` 에서 그 줄 삭제.
- ⚠️ **CSS 클래스는 전부 `rm-` 접두사로.** 빈 달 카드에 `empty` 를 썼다가 globals.css 의
  전역 `.empty`(가운데 정렬 빈 상태)에 걸려 라벨이 통째로 가운데로 끌려갔다. 지금은 `blank`.
  같은 이유로 상태 팔레트(`is-done`/`is-doing`/…)도 **`.rm-page` 로 스코프**한다.
- ⚠️ **화면에 사용법을 설명하는 문구를 두지 말 것.** 편집 화면에 "시점은 아는 만큼만 적습니다 —
  … 범위가 넓을수록 마커도 넓게 그려집니다" 같은 안내를 달았다가 **빠졌다**. 형식은 입력 위젯이
  스스로 말하고, 설계 의도 설명은 코드 주석과 이 문서에만 둔다.
...
- `src/lib/tempStatus.ts` — 아래 export 들이 모두 임시 코드 (파일 전체 삭제 대상):
  ...
  - `ACTION_FAIL_LABELS` — 가상 코드 → 사람이 읽는 라벨 (`/api/insights` 의 "주요 실패 원인" 표기용)
...
4. `src/app/api/stats/route.ts` 의 트레이스 루프에서 액션 실패 Top Errors 보정 블록 삭제
   (⚠️ `src/app/api/insights/route.ts` 도 `ACTION_FAIL_LABELS` 를 import 한다 — 같이 정리할 것.
