...
- **실시간(1TICK) 창은 `TimeRangeProvider` 가 아니라 `TickProvider` 가 쥔다** — 격자(초/분)도
  응답 형태도 달라 같은 상태에 담을 수 없다. 다만 성격이 같아 **Dashboard/Tokens/Timeout 셋이
  공유**한다(`localStorage["tracex.tick"]`). 자세한 건 아래 "실시간(1TICK)" 절.
- **Dashboard/Report/Insights 는 공유 대상이 아니다** — 기간 개념이 다르다(주 단위 이동, 월~월 경계 등).

### 실시간(1TICK) — 롤링 60초 모니터 (Dashboard · Tokens · Timeout 공용)
...
- ⚠️ **BIZ 집계는 진입 레이어(`LAYER_ORDER[0]`) 하나만 읽는다** (`src/lib/bizTickStats.ts`). 사용자 요청
  ...
  (`ERR_CD` 또는 TEMP 액션 실패 문구 — 여기만 `ERR_CD` 로 좁히면 대시보드 KPI 와 실시간 뷰의 실패 수가 갈린다).
  ⚠️ `ACTION_TYP`/`FAC_ID` 는 **담기지 않는다** — 다른 레이어가 기록하는 값이라 진입 레이어 행에는 없다.
  그래서 대시보드의 ACTION_TYP 셀렉트는 실시간 뷰에서 감춘다(걸어도 효과가 없는데 걸린 것처럼 보이면
  "이 액션은 요청이 없다" 로 오독된다).
...
**UI — 보기 토글 (`src/components/tick/`)**

- ⚠️ **진입은 기간 프리셋 줄 밖의 `ViewToggle`(`기간 분석 | ● 실시간`)이다.** 예전엔 프리셋 줄 **안의**
  `1TICK` 버튼이었는데, 그 줄의 다른 버튼은 전부 "조회 기간" 인데 이것만 화면을 통째로 바꿔서
  기간을 고른 줄 알고 눌렀다가 화면이 달라지는 일이 있었다. 성격이 다른 조작은 줄 밖에 둔다.
- ⚠️ **기간 프리셋 줄 자리는 뷰에 따라 통째로 교체된다** — 실시간이면 그 자리에 `TickToolbar`
  (창 길이 + 직접 설정 + 자동 갱신 + 새로고침)가 들어간다. 두 줄을 같이 띄우면 "기간을 골랐는데
  왜 안 바뀌지" 가 다시 생긴다.
- **`● 실시간` 의 점은 자동 갱신이 도는 동안에만 깜빡인다**(`.live-dot.pulse`) — 지금 살아 있는 값인지
...
- **공유 범위**: 창 길이·모드·직접 설정 구간·자동 여부는 `TickProvider` 가 세 화면과 **공유**하고,
  **뷰 on/off 만 화면별**이다(`useTickView(key)` → `localStorage["tracex.tick.view.<key>"]`).
  대시보드를 실시간으로 띄워 두고 Tokens 는 기간 분석으로 보는 조합이 정상이기 때문.
...
- **조회 툴바는 `TickMonitor` 안에 없다** — 페이지 헤더의 `TickToolbar` 가 담당한다. 두 줄로 나뉘어
  있으면 어느 쪽이 지금 조회 조건인지 헷갈린다.
- 기존 USER/NODE/MODEL 필터는 그대로 적용된다(각 페이지가 현재 뷰에 맞는 쪽을 다시 조회).
- **실시간 뷰가 아닐 때 세 화면은 기존 경로 그대로다** — 기존 화면 로직은 바뀌지 않았다.
...
- **실시간 뷰**: 상단 `기간 분석 | 실시간` 토글로 **분당 타임아웃/실패**(롤링 60초)를 본다 —
  위 "실시간(1TICK)" 절 참고. ⚠️ Tokens 탭의 TPM/RPM 을 복제하지 않는다(같은 숫자가 두 번 나온다).
