...
### 처리 추이의 차트 단위
[tick.md](tick.md) 의 `집계 ┊ 틱` 세그먼트를 카드 머리(`Card` 의 `aux`)에 단다
(`useTickUnit("insights", …)`, 선택지는 조회 구간 길이가 정한다).

- `/api/insights?g=` 는 **`computeStats()` 에만** 넘긴다 — 토큰·타임아웃 추이는 집계 격자 그대로다
- `gran` 은 버킷만 바꾼다. KPI·일별·기능별 등 나머지 수치와 리포트 텍스트는 단위와 무관하다
- 1분도 정각 분 격자다(롤링 60초는 Tokens 전용). 자동 갱신은 이 화면에 없다
